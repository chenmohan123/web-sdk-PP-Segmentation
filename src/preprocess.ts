...
export function validatePixels(image: PixelImage): void {
  if (
    ...
    throw new SegmentationError(
      "INVALID_INPUT",
      "图片需要正整数尺寸、完整RGBA数据且不超过16777216像素",
    );
  }
}
...
function cubicAxis(size: number) {
  const indices = new Int32Array(640 * 4),
    weights = new Float64Array(640 * 4);
  for (let x = 0; x < 640; x++) {
    const source = ((x + 0.5) * size) / 640 - 0.5,
      base = Math.floor(source);
    for (let k = 0; k < 4; k++) {
...
export function preprocess(image: PixelImage): Float32Array {
  ...
  try {
    const horizontal = cubicAxis(image.width),
      vertical = cubicAxis(image.height);
    ...
    for (let y = 0; y < 640; y++) {
      for (let k = 0; k < 4; k++) {
        const sourceY = vertical.indices[y * 4 + k],
          slot = sourceY % 4;
        ...
        for (let x = 0; x < 640; x++)
          for (let c = 0; c < 3; c++) {
            ...
            for (let j = 0; j < 4; j++) {
              const pixel =
                (sourceY * image.width + horizontal.indices[x * 4 + j]) * 4;
              const alpha = image.data[pixel + 3] / 255;
              value +=
                (image.data[pixel + c] * alpha + 255 * (1 - alpha)) *
                horizontal.weights[x * 4 + j];
            }
            row[c * 640 + x] = value;
          }
      }
      for (let c = 0; c < 3; c++)
        for (let x = 0; x < 640; x++) {
          ...
          for (let k = 0; k < 4; k++)
            value +=
              rows[vertical.indices[y * 4 + k] % 4][c * 640 + x] *
              vertical.weights[y * 4 + k];
          result[c * 409600 + y * 640 + x] =
            Math.min(255, Math.max(0, Math.round(value))) / 255;
        }
    }
    return result;
  } catch (error) {
    throw wrapError(error, "OUT_OF_MEMORY", "预处理缓冲区分配失败");
  }
}
