...
| `DOWNLOAD`                                  | Inspect HTTP status, CORS, redirects, connectivity, and CSP `connect-src` in Network tools. Ensure local `.tmp/model.onnx` exists; use `models/model.json` for remote sources                                                          |
...
| Run button unavailable                      | Select a valid image and check that the selected source is complete in the model manifest. Production uses pinned Hub sources; development uses an explicit local model.                                                               |
...
| An old report says acceptance failed        | The old official reference truncates the last row/column; the current original-integer-size reference passes. Check the report path and criteria; see [Compatibility](compatibility.md)                                                |
